export const AppHandlers = {
 ...
 // 中文格式优化器（中英文混排 & 标点统一 & 去除中文标点后的空格）
zhFormatterV1(text = '', preset = '标准', ctx = {}) {
  // 模式：仅标点=punct；严格=strict；默认=standard
  ...
  const unifyPunct = (t) => {
    ...
    // 逗号：不影响 1,234 这种数字分组
    t = t.replace(/(^|[^\d]),(?=[^\d]|$)/g, '$1，').replace(/,(\s|$)/g, '，$1');

    // 分号/问号/叹号
    t = t.replace(/;/g, '；').replace(/\?/g, '？').replace(/!/g, '！');

    // 冒号：统一为全角，再把时间 12:34 还原
  ...
  /* ---------- 3) 空格规则 ---------- */
  // A. 核心修复：中文标点后的空格，一律去掉
  const trimAfterCnPunct = (t) => {
    // 中文标点后空格
    t = t.replace(/([，。；：？！、])\s+/g, '$1');
    // 右引号/右括号/书名号等后的空格
    t = t.replace(/([”’）》】）])\s+/g, '$1');
  ...
  const spacing = (t, strong = false) => {
    ...
    // 标点前不留空、开口标点后不留空
    t = t.replace(/\s+([，。；：？！、》】）])/g, '$1');  // 标点前空格
    t = t.replace(/([（【《「『])\s+/g, '$1');           // 开口标点后空格
  ...
  function smartQuotes(t) {
    ...
    for (let i = 0; i < t.length; i++) {
      ...
      if (ch === "'") {
        if (/\w/.test(prev || '') && /\w/.test(next || '')) { out += '’'; continue; } // don't
  ...
  // 顺序：统一标点 → 去中文标点后的空格（关键）→（可选）混排空格
  s = unifyPunct(s);
  s = trimAfterCnPunct(s);
  if (mode === 'standard') s = spacing(s, false);
  if (mode === 'strict')   s = spacing(s, true);
  // 'punct' 模式不做 spacing，但已做了标点后的空格清理
